...
/* Q1. Write a query to display customer_id, customer full name with their title (Mr/Ms), both first name and last name are in upper case, 
customer_email, customer_creation_year and display customer’s category after applying below categorization rules: i. if
 CUSTOMER_CREATION_DATE year <2005 then category A ii. if CUSTOMER_CREATION_DATE year >=2005 and <2011 then category B 
 iii. if CUSTOMER_CREATION_DATE year>= 2011 then category C Expected 52 rows in final output. 
 [Note: TABLE to be used - ONLINE_CUSTOMER TABLE] Hint:Use CASE statement. create customer_creation_year column with the help 
 of customer_creation_date, no permanent change in the table is required. (Here don’t UPDATE or DELETE the columns in the table nor 
 CREATE new tables for your representation. A new column name can be used as an alias for your manipulation in case if you are going 
 to use a CASE statement.)*/

SELECT CUSTOMER_ID,

/*Concatinating name with title(Mr,Mrs)and first name and last name in upper case*/
CONCAT(IF(CUSTOMER_GENDER='F','MRS.','MR.'),' ', UPPER(CUSTOMER_FNAME) , ' ' , UPPER(CUSTOMER_LNAME)) AS CUSTOMER_FULLNAME ,

CUSTOMER_EMAIL,

/*Extracting date from Customer_Creation_Date*/
...
/*Q2)Write a query to display the following information for the products which have not been sold: product_id, product_desc, 
...
/*ANS*/
SELECT PRODUCT.PRODUCT_ID,PRODUCT_DESC,PRODUCT_QUANTITY_AVAIL,PRODUCT_PRICE,
(PRODUCT_QUANTITY_AVAIL*PRODUCT_PRICE) AS INVENTORY_VALUES,
CASE
    WHEN PRODUCT_PRICE > 20000 THEN ((PRODUCT_PRICE*80)/100)
    WHEN PRODUCT_PRICE > 10000 AND PRODUCT_PRICE <= 20000 THEN ((PRODUCT_PRICE*85)/100)
    WHEN PRODUCT_PRICE <= 10000 THEN ((PRODUCT_PRICE*90)/100)
END AS NEW_PRICE
FROM PRODUCT
WHERE PRODUCT.PRODUCT_ID NOT IN (SELECT PRODUCT_ID FROM ORDER_ITEMS)
ORDER BY INVENTORY_VALUES DESC;




/*Q3. Write a query to display Product_class_code, Product_class_desc, Count of Product type in each product class, Inventory Value
 (p.product_quantity_avail*p.product_price). Information should be displayed for only those product_class_code which have more than 
 1,00,000 Inventory Value. Sort the output with respect to decreasing value of Inventory_Value. Expected 9 rows in final output. 
 [NOTE: TABLES to be used - PRODUCT, PRODUCT_CLASS] Hint: 'count of product type in each product class' is the count of product_id based
 on product_class_code.*/


/*ANS*/

SELECT PRODUCT_CLASS.PRODUCT_CLASS_CODE,PRODUCT_CLASS_DESC,
SUM(PRODUCT_QUANTITY_AVAIL*PRODUCT_PRICE) AS INVENTORY_VALUES,
COUNT(PRODUCT.PRODUCT_ID) AS PRODUCT_ID_COUNT
FROM PRODUCT_CLASS
CROSS JOIN PRODUCT
ON PRODUCT_CLASS.PRODUCT_CLASS_CODE=PRODUCT.PRODUCT_CLASS_CODE
WHERE ((PRODUCT_QUANTITY_AVAIL*PRODUCT_PRICE) > 100000  )
GROUP BY PRODUCT_CLASS_CODE
ORDER BY INVENTORY_VALUES DESC;

/*Q4. Write a query to display customer_id, full name, customer_email, customer_phone and country of customers who have cancelled
 all the orders placed by them. Expected 1 row in the final output [NOTE: TABLES to be used - ONLINE_CUSTOMER, ADDRESSS, OREDER_HEADER]
 Hint: USE SUBQUERY*/
 ...
 CONCAT(IF(CUSTOMER_GENDER='F','Mrs.','Ms.'),' ',CUSTOMER_FNAME, ' ' , CUSTOMER_LNAME) AS CUSTOMER_FULLNAME,
 CUSTOMER_EMAIL,
 CUSTOMER_PHONE,
 ADDRESS.COUNTRY
 FROM ONLINE_CUSTOMER
...
number of consignment delivered to that city for Shipper DHL Expected 9 rows in the final output [NOTE: TABLES to be used 
- SHIPPER, ONLINE_CUSTOMER, ADDRESSS, ORDER_HEADER] Hint: The answer should only be based on Shipper_Name -- DHL. 
...
SELECT SHIPPER_NAME,ADDRESS.CITY,COUNT(ORDER_HEADER.CUSTOMER_ID) AS CUSTOMER_NUMBER,COUNT(CITY) AS CONSIGNMENT
FROM ORDER_HEADER 
JOIN SHIPPER
ON ORDER_HEADER.SHIPPER_ID=SHIPPER.SHIPPER_ID
...
/*Q6. Write a query to display product_id, product_desc, product_quantity_avail, quantity sold and show inventory Status of products 
as per below condition: a. For Electronics and Computer categories, if sales till date is Zero then show 'No Sales in past, give discount 
to reduce inventory', if inventory quantity is less than 10% of quantity sold, show 'Low inventory, need to add inventory',
 if inventory quantity is less than 50% of quantity sold, show 'Medium inventory, need to add some inventory', if inventory quantity is 
 more or equal to 50% of quantity sold, show 'Sufficient inventory' b. For Mobiles and Watches categories, if sales till date is Zero then 
 show 'No Sales in past, give discount to reduce inventory', if inventory quantity is less than 20% of quantity sold, show 'Low inventory,
 need to add inventory', if inventory quantity is less than 60% of quantity sold, show 'Medium inventory, need to add some inventory',
 if inventory quantity is more or equal to 60% of quantity sold, show 'Sufficient inventory' c. Rest of the categories, if sales till 
 date is Zero then show 'No Sales in past, give discount to reduce inventory', if inventory quantity is less than 30% of quantity sold, 
 show 'Low inventory, need to add inventory', if inventory quantity is less than 70% of quantity sold, show 'Medium inventory, need to 
 add some inventory', if inventory quantity is more or equal to 70% of quantity sold, show 'Sufficient inventory' Expected 60 rows in
 final output [NOTE: (USE CASE statement) ; TABLES to be used - PRODUCT, PRODUCT_CLASS, ORDER_ITEMS] Hint: quantity sold here is 
 product_quantity in order_items table. You may use multiple case statements to show inventory status (Low stock, In stock, and Enough stock)
 that meets both the conditions i.e. on products as well as on quantity. The meaning of the rest of the categories, means products apart 
 from electronics, computers, mobiles, and watches.*/


/*ANS*/

SELECT DISTINCT(PRODUCT.PRODUCT_ID) AS PRODUCTID,PRODUCT_DESC,PRODUCT.PRODUCT_QUANTITY_AVAIL, (SELECT SUM(PRODUCT_QUANTITY) FROM ORDER_ITEMS WHERE PRODUCT_ID=PRODUCTID) AS QUANTITY_SOLD,
CASE 
    WHEN PRODUCT.PRODUCT_CLASS_CODE="ELECTRONICS" AND "COMPUTER" THEN
         CASE
             WHEN PRODUCT_QUANTITY=0 THEN 'No Sales in past, give discount to reduce inventory'
             WHEN PRODUCT_QUANTITY_AVAIL < (0.1* PRODUCT_QUANTITY) THEN 'Low inventory, need to add inventory'
             WHEN PRODUCT_QUANTITY_AVAIL < (0.5* PRODUCT_QUANTITY) AND PRODUCT_QUANTITY_AVAIL > (0.1* PRODUCT_QUANTITY) THEN 'Medium inventory, need to add some inventory'
             ELSE 'Sufficient inventory'
           END
	WHEN  PRODUCT.PRODUCT_CLASS_CODE="MOBILE" AND "WATCHES" THEN
		 CASE
             WHEN PRODUCT_QUANTITY=0 THEN 'No Sales in past, give discount to reduce inventory'
             WHEN PRODUCT_QUANTITY_AVAIL < (0.2* PRODUCT_QUANTITY) THEN 'Low inventory, need to add inventory'
             WHEN PRODUCT_QUANTITY_AVAIL < (0.6* PRODUCT_QUANTITY) AND PRODUCT_QUANTITY_AVAIL >(0.2* PRODUCT_QUANTITY) THEN 'Medium inventory, need to add some inventory'
             ELSE 'Sufficient inventory'
          END
	ELSE   
         CASE
             WHEN PRODUCT_QUANTITY=0 THEN 'No Sales in past, give discount to reduce inventory'
             WHEN PRODUCT_QUANTITY_AVAIL < (0.3* PRODUCT_QUANTITY) THEN 'Low inventory, need to add inventory'
             WHEN PRODUCT_QUANTITY_AVAIL < (0.7* PRODUCT_QUANTITY) AND PRODUCT_QUANTITY_AVAIL > (0.3* PRODUCT_QUANTITY) THEN 'Medium inventory, need to add some inventory'
             ELSE'Sufficient inventory'
          END
...
/*Q7. Write a query to display order_id and volume of the biggest order (in terms of volume) that can fit in carton id 10 .
 Expected 1 row in final output [NOTE: TABLES to be used - CARTON, ORDER_ITEMS, PRODUCT] Hint: First find the volume of carton id 10 
 and then find the order id with products having total volume less than the volume of carton id 10*/
 

 
 /*ANS*/
 
 SELECT ORDER_ID,(PRODUCT.LEN*PRODUCT.WIDTH*PRODUCT.HEIGHT) AS BIGGEST_ORDER
 FROM ORDER_ITEMS
 JOIN PRODUCT
 ON ORDER_ITEMS.PRODUCT_ID=PRODUCT.PRODUCT_ID
 WHERE (PRODUCT.LEN*PRODUCT.WIDTH*PRODUCT.HEIGHT) >(SELECT (CARTON.LEN*CARTON.WIDTH*CARTON.HEIGHT) FROM CARTON WHERE CARTON_ID=10)
 ...
 shipped where mode of payment is Cash and customer last name starts with 'G' Expected 2 rows in final output 
...
JOIN PRODUCT
ON ORDER_ITEMS.PRODUCT_ID=PRODUCT.PRODUCT_ID
WHERE ORDER_HEADER.PAYMENT_MODE='CASH' AND ORDER_HEADER.ORDER_STATUS='SHIPPED' AND ONLINE_CUSTOMER.CUSTOMER_LNAME LIKE 'G%'
...
and are not shipped to city Bangalore and New Delhi. [NOTE: TABLES to be used - ORDER_ITEMS, PRODUCT, ORDER_HEADER, ONLINE_CUSTOMER, ADDRESS]
 Hint: Display the output in descending order with respect to the sum of product_quantity. (USE SUB-QUERY) In final output show only those 
 products , product_id’s which are sold with 201 product_id (201 should not be there in output) and are shipped except Bangalore and New Delhi*/
 
   
 /*ANS*/
 
 SELECT PRODUCT.PRODUCT_ID AS PRODUCTID,PRODUCT_DESC,
 (SELECT SUM(PRODUCT_QUANTITY) FROM ORDER_ITEMS WHERE PRODUCT_ID=PRODUCTID) AS TOTAL_QUANTITY
 FROM PRODUCT
 JOIN ORDER_ITEMS
 ON PRODUCT.PRODUCT_ID=ORDER_ITEMS.PRODUCT_ID
 JOIN ORDER_HEADER
 ...
 AND CITY !='BANGALORE' AND CITY !='NEW DELHI';
...
/*Q10. Write a query to display the order_id, customer_id and customer fullname, total quantity of products shipped for order ids
 which are even and shipped to address where pincode is not starting with "5" Expected 15 rows in final output 
 [NOTE: TABLES to be used - ONLINE_CUSTOMER, ORDER_HEADER, ORDER_ITEMS, ADDRESS]*/
 
 SELECT GROUP_CONCAT(ORDER_ITEMS.ORDER_ID),ONLINE_CUSTOMER.CUSTOMER_ID,
 CONCAT(IF(CUSTOMER_GENDER='F','Mrs.','Ms.'),' ',CUSTOMER_FNAME, ' ' , CUSTOMER_LNAME) AS CUSTOMER_FULLNAME,
 SUM(ORDER_ITEMS.PRODUCT_QUANTITY) AS TOTAL_QUANTITY
 FROM ONLINE_CUSTOMER
 JOIN ORDER_HEADER
 ...
 JOIN ADDRESS
 ON ONLINE_CUSTOMER.ADDRESS_ID=ADDRESS.ADDRESS_ID
 WHERE (ORDER_ITEMS.ORDER_ID % 2 =0)
 AND ADDRESS.PINCODE NOT LIKE '5%'
